...
describe('order utilities', () => {
  ...
  describe('canCancelOrder', () => {
    ...
    it('should allow cancel for awaiting_payment order within window', () => {
      const createdAt = new Date('2025-01-18T09:00:00Z');
      const result = canCancelOrder('awaiting_payment', createdAt, 24);
      expect(result.canCancel).toBe(true);
    });

    it('should not allow cancel for paid order', () => {
      const createdAt = new Date('2025-01-18T09:00:00Z');
      const result = canCancelOrder('paid', createdAt, 24);
