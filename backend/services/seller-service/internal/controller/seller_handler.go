...
func (h *SellerHandler) RegisterRoutes(r *mux.Router) {
	...
	r.HandleFunc("/{sellerId}/orders", h.ListSellerOrders).Methods("GET")
	r.HandleFunc("/{sellerId}/orders/{orderId}", h.GetSellerOrder).Methods("GET")
	r.HandleFunc("/{sellerId}/orders/{orderId}/confirm", h.ConfirmSellerOrder).Methods("POST")
	r.HandleFunc("/{sellerId}/orders/{orderId}/ship", h.ShipSellerOrder).Methods("POST")
	r.HandleFunc("/{sellerId}/orders/{orderId}/tracking", h.UpdateSellerOrderTracking).Methods("PATCH")
}
...
// @Description List orders for a seller (proxy to order-service).
// @Tags Orders
// @Produce json
// @Param sellerId path string true "Seller ID"
...
// @Param q query string false "Search term (order number or buyer name)"
// @Param page query int false "Page number"
// @Param limit query int false "Page size"
// @Success 200 {object} types.ListSellerOrdersResponseDTO
// @Failure 500 {object} map[string]interface{}
// @Router /{sellerId}/orders [get]
func (h *SellerHandler) ListSellerOrders(w http.ResponseWriter, r *http.Request) {
	sellerID := mux.Vars(r)["sellerId"]
	q := r.URL.Query()
	...
	resp, err := h.service.ListSellerOrders(r.Context(), sellerID, params)
	if err != nil {
		writeError(w, http.StatusInternalServerError, err.Error())
		return
	}
...
// @Description Get a single order detail for a seller (proxy to order-service).
// @Tags Orders
// @Produce json
// @Param sellerId path string true "Seller ID"
// @Param orderId path string true "Order ID"
// @Success 200 {object} types.OrderDetailResponseDTO
// @Failure 500 {object} map[string]interface{}
// @Router /{sellerId}/orders/{orderId} [get]
func (h *SellerHandler) GetSellerOrder(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	sellerID := vars["sellerId"]
	orderID := vars["orderId"]

	resp, err := h.service.GetSellerOrder(r.Context(), sellerID, orderID)
	if err != nil {
		writeError(w, http.StatusInternalServerError, err.Error())
		return
	}
...
// @Description Confirm an order for a seller (proxy to order-service).
// @Tags Orders
// @Param sellerId path string true "Seller ID"
// @Param orderId path string true "Order ID"
// @Success 200 {object} map[string]interface{}
// @Failure 500 {object} map[string]interface{}
// @Router /{sellerId}/orders/{orderId}/confirm [post]
func (h *SellerHandler) ConfirmSellerOrder(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	sellerID := vars["sellerId"]
	orderID := vars["orderId"]

	if err := h.service.ConfirmSellerOrder(r.Context(), sellerID, orderID); err != nil {
		writeError(w, http.StatusInternalServerError, err.Error())
		return
	}
...
// @Summary Ship Seller Order
// @Description Mark an order as shipped for a seller (proxy to order-service).
// @Tags Orders
// @Param sellerId path string true "Seller ID"
// @Param orderId path string true "Order ID"
// @Success 200 {object} map[string]interface{}
// @Failure 500 {object} map[string]interface{}
// @Router /{sellerId}/orders/{orderId}/ship [post]
func (h *SellerHandler) ShipSellerOrder(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	sellerID := vars["sellerId"]
	orderID := vars["orderId"]

	if err := h.service.ShipSellerOrder(r.Context(), sellerID, orderID); err != nil {
		writeError(w, http.StatusInternalServerError, err.Error())
		return
	}
...
// @Summary Update Seller Order Tracking
// @Description Update tracking number for an order (proxy to order-service).
// @Tags Orders
// @Accept json
// @Produce json
// @Param sellerId path string true "Seller ID"
// @Param orderId path string true "Order ID"
// @Param payload body map[string]string true "Tracking payload"
// @Success 200 {object} map[string]interface{}
// @Failure 400 {object} map[string]interface{}
// @Failure 500 {object} map[string]interface{}
// @Router /{sellerId}/orders/{orderId}/tracking [patch]
func (h *SellerHandler) UpdateSellerOrderTracking(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	sellerID := vars["sellerId"]
	orderID := vars["orderId"]
	...
	if err := h.service.UpdateSellerOrderTracking(r.Context(), sellerID, orderID, body.TrackingNumber); err != nil {
		writeError(w, http.StatusInternalServerError, err.Error())
		return
	}
