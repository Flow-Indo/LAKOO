    ...
    <div className="bg-gradient-to-r from-red-500 to-red-600 text-white px-3 sm:px-4 py-2">
      ...
      <div className="flex items-center justify-between mb-2">
        <div className="text-xs sm:text-sm">
          ...
          <span className="font-bold text-lg sm:text-2xl">{product.price?.toLocaleString('id-ID')}</span>
      ...
      <div className="flex items-center gap-1 sm:gap-2">
        <div className="bg-white text-red-600 px-1.5 sm:px-2 py-1 rounded text-xs font-bold flex items-center gap-1 flex-1 min-w-0">
          <span>↓</span>
          <span className="truncate">Diskon {product.discountPercentage}% Hemat Rp {((product.originalPrice || 0) - (product.price || 0)).toLocaleString('id-ID')}</span>
        </div>
